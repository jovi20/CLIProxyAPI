...
class ChatService:
    ...
    async def set_model(self):
        self.raw_model = self.data.get("model", "gpt-3.5-turbo-0125")
        self.origin_model, self.model_suffix = split_model_name_and_suffix(self.raw_model)
        self.resp_model = model_proxy.get(self.origin_model, self.raw_model)
        self.reasoning_effort = extract_reasoning_effort(self.data, self.model_suffix)
        if "gizmo" in self.origin_model or "g-" in self.origin_model:
            self.gizmo_id = "g-" + self.origin_model.split("g-")[-1]
        else:
            self.gizmo_id = None

        self.req_model = resolve_request_model(self.origin_model)
    ...
    async def prepare_send_conversation(self):
        ...
        logger.info(f"Model mapping: {self.raw_model} -> {self.req_model}")
        ...
        if self.reasoning_effort:
            self.chat_request["reasoning_effort"] = self.reasoning_effort
